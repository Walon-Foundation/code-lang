...
use code_lang::{analysis::scope::ScopeTree, ast::{ast::{Expression, LetPattern, Statement}, walk::{Visitor, walk_expression, walk_statement}}, token::token::TokenType};
        ...
        for name in names {
            ...
            if let Some((prev_line, prev_col)) = scope.get(&name) {
                self.diags.push(LintDiagnostic {
                    ...
                    message: format!("'{}' shadows an earlier binding at {}:{}", name, prev_line, prev_col),
                    line,
                    colum: col,
                    severity: LintSeverity::Warning,
                    fix: None,
    ...
    fn diagnostic(&mut self) -> &[LintDiagnostic] {
        ...
        for (name, (line, col, is_ident)) in &self.declared {
            if !self.used.contains(&name.to_string()){
                ...
                let fix = if *is_ident {
                    Some(LintFix {
                        start_line: *line, start_col: col + 3,
                        end_line:   *line, end_col:   col + 3 + name.len(),
...
impl Visitor for ConstReassignment {
    fn visit_const(&mut self, pattern: &LetPattern, _value: &Expression, _line: usize, _col: usize) {
        let names: Vec<String> = match pattern {
            LetPattern::Ident(n) => vec![n.clone()],
            LetPattern::Array(names) => names.clone(),
            LetPattern::Hash(pairs) => pairs.iter().map(|(_, alias)| alias.clone()).collect(),
    ...
    fn visit_infix(&mut self, op: TokenType, left: &Expression, _right: &Expression,
                   line: usize, col: usize)
    {
        if let TokenType::Assign = op {
            if let Expression::Ident { value: name, .. } = left {
    ...
    fn visit_update(&mut self, operator: &code_lang::token::token::Token, target: &Expression, line: usize, col: usize) {
        if let Expression::Ident { value: name, .. } = target {
            if self.consts.contains(name) {
                self.diags.push(LintDiagnostic {
                    rule: "const-reassignment",
                    message: format!("cannot update const '{}' with operator {:?}", name, operator.token_type),
                    line,
                    colum: col,
                    severity: LintSeverity::Error,
                    fix: None,
...
impl Visitor for EmptyBlock {
    fn visit_expression(&mut self, expr: &Expression) {
        if let Expression::If { consequence, .. } = expr {
            if let Statement::Block { statements, line, column, .. } = &**consequence {
                if statements.is_empty() {
                    self.diags.push(LintDiagnostic{
                        ...
                        colum: *column,
                        rule: "empty-block",
                        message: format!("empty if body - did you forget to add statments"),
                        fix:None,
                    });
                }
            }
        ...
        if let Expression::While { body, .. } = expr {
            if let Statement::Block { statements, line, column, .. } = &**body {
                if statements.is_empty() {
                    self.diags.push(LintDiagnostic{
                        severity:LintSeverity::Warning,
                        line: *line,
                        colum: *column,
                        rule: "empty-block",
                        message: format!("empty while body - did you forget to add statments"),
                        fix:None,
                    });
                }
            }
            ...
            if let Statement::Block { statements, line, column, .. } = &**body {
                if statements.is_empty() {
                    self.diags.push(LintDiagnostic{
                        ...
                        colum: *column,
                        rule: "empty-block",
                        message: format!("empty for body - did you forget to add statments"),
                        fix:None,
                    });
                }
            }
            ...
            if let Statement::Block { statements, line, column, .. } = &**body {
                if statements.is_empty() {
                    self.diags.push(LintDiagnostic{
                        ...
                        colum: *column,
                        rule: "empty-block",
                        message: format!("empty function body - did you forget to add statments"),
                        fix:None,
                    });
                }
            }
            ...
            if let Statement::Block { statements, line, column, .. } = &**body {
                if statements.is_empty() {
                    self.diags.push(LintDiagnostic{
                        ...
                        colum: *column,
                        rule: "empty-block",
                        message: format!("empty for-in body - did you forget to add statments"),
                        fix:None,
                    });
                }
            }
...
// ----------------------------------- undefined variable ----------------------------------------
const BUILTINS: &[&str] = &[
    "fmt", "arrays", "math", "strings", "hash", "json",
    "fs", "os", "path", "rand", "time", "http",
