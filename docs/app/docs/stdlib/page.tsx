...
const MODULES: {
...
}[] = [
  {
    ...
    fns: [
      ["print(...args)", "Print args space-separated to stdout with newline.", "null"],
      ["eprint(...args)", "Same as print but to stderr.", "null"],
  ...
  },
  {
    ...
    fns: [
      ...
      ["trim(s) / trim_left(s) / trim_right(s)", "Remove whitespace.", "string"],
      ["split(s, sep)", "Split string on sep.", "[string]"],
      ["join(arr, sep)", "Join array into a string.", "string"],
      ...
      ["index(s, sub)", "First index of sub in s, or -1.", "integer"],
      ["count(s, sub)", "Number of non-overlapping occurrences of sub.", "integer"],
      ["repeat(s, n)", "Repeat s n times.", "string"],
      ["reverse(s)", "Reverse the string.", "string"],
      ["to_chars(s)", "Split s into individual characters.", "[char]"],
      ["from_chars(arr)", "Build a string from an array of chars.", "string"],
      ["parse_int(s) / parse_float(s)", "Parse string to number.", "integer | float"],
      ["lines(s)", "Split by newline.", "[string]"],
      ["is_empty(s)", "True if string has zero characters.", "bool"],
      ["pad_left(s, n, ch)", "Left-pad s to width n using char ch.", "string"],
      ["pad_right(s, n, ch)", "Right-pad s to width n using char ch.", "string"],
    ],
  },
  {
    name: "arrays",
    ...
    fns: [
      ...
      ["index_of(arr, x)", "First index of x, or -1.", "integer"],
      ["join(arr, sep)", "Join elements into a string.", "string"],
      ["sum(arr)", "Sum of numeric elements.", "integer | float"],
      ["min(arr) / max(arr)", "Min / max of a numeric array.", "integer | float"],
      ...
      ["zip(a, b)", "Pair elements: a[i] with b[i], stops at shorter.", "[[any, any]]"],
      ["map(arr, fn)", "New array of fn(element) results.", "[any]"],
      ["filter(arr, fn)", "Elements where fn(element) is truthy.", "[any]"],
      ["reduce(arr, fn, init)", "Accumulate fn(acc, element) left-to-right from init.", "any"],
