...
fn typeof_fn(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "fmt.typeof takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
...
fn to_int(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "fmt.to_int takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    match &args[0] {
        ...
        _ => Object::Error {
            message: format!(
                "fmt.to_int: cannot convert {} to integer",
                args[0].type_name()
            ),
            line: info.line,
            column: info.column,
...
fn to_float(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "fmt.to_float takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    match &args[0] {
        ...
        Object::StringType(s) => match s.trim().parse::<f64>() {
            Ok(f) => Object::Float(f),
            Err(_) => Object::Error {
                message: format!("fmt.to_float: cannot convert \"{}\" to float", s),
                line: info.line,
                column: info.column,
            },
        },
        _ => Object::Error {
            message: format!(
                "fmt.to_float: cannot convert {} to float",
                args[0].type_name()
            ),
            line: info.line,
            column: info.column,
...
fn to_str(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "fmt.to_str takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
...
fn input(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "fmt.input takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    match &args[0] {
        Object::StringType(prompt) => {
            ...
            match stdin.lock().read_line(&mut line) {
                Ok(_) => Object::StringType(
                    line.trim_end_matches('\n')
                        .trim_end_matches('\r')
                        .to_string(),
                ),
                Err(e) => Object::Error {
                    message: format!("fmt.input: {}", e),
                    line: info.line,
                    column: info.column,
                },
            }
        ...
        _ => Object::Error {
            message: format!(
                "fmt.input expects STRING prompt, got {}",
                args[0].type_name()
            ),
            line: info.line,
            column: info.column,
...
fn clear(args: Vec<Object>, info: CallInfo) -> Object {
    if !args.is_empty() {
        return Object::Error {
            message: "fmt.clear takes no arguments".to_string(),
            line: info.line,
            column: info.column,
        };
    }
...
fn format_fn(args: Vec<Object>, info: CallInfo) -> Object {
    if args.is_empty() {
        return Object::Error {
            message: "fmt.format expects at least 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    let template = match &args[0] {
        ...
        _ => {
            return Object::Error {
                message: format!(
                    "fmt.format: first argument must be STRING, got {}",
                    args[0].type_name()
                ),
                line: info.line,
                column: info.column,
