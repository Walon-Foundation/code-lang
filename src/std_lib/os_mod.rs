...
fn get_env(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 1 {
        return Object::Error {
            message: "os.get_env takes 1 argument".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    match &args[0] {
        ...
        _ => Object::Error {
            message: format!("os.get_env expects STRING, got {}", args[0].type_name()),
            line: info.line,
            column: info.column,
        },
    }
...
fn set_env(args: Vec<Object>, info: CallInfo) -> Object {
    if args.len() != 2 {
        return Object::Error {
            message: "os.set_env takes 2 arguments".to_string(),
            line: info.line,
            column: info.column,
        };
    }
    let key = match &args[0] {
        ...
        _ => {
            return Object::Error {
                message: format!(
                    "os.set_env: key must be STRING, got {}",
                    args[0].type_name()
                ),
                line: info.line,
                column: info.column,
    ...
    let val = match &args[1] {
        ...
        _ => {
            return Object::Error {
                message: format!(
                    "os.set_env: value must be STRING, got {}",
                    args[1].type_name()
                ),
                line: info.line,
                column: info.column,
...
fn get_wd(args: Vec<Object>, info: CallInfo) -> Object {
    if !args.is_empty() {
        return Object::Error {
            message: "os.get_wd takes no arguments".to_string(),
            line: info.line,
            column: info.column,
        };
    }
...
pub fn module() -> Object {
    let args: Vec<Object> = std::env::args().map(Object::StringType).collect();

    let mut members: HashMap<String, Object> = HashMap::new();
    members.insert("args".to_string(), Object::Array(args));
    members.insert(
        "platform".to_string(),
        Object::StringType(std::env::consts::OS.to_string()),
    );
    members.insert(
        "arch".to_string(),
        Object::StringType(std::env::consts::ARCH.to_string()),
    );
